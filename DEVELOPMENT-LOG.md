...
## 2026-07-22, Security & privacy audit re-run (repeat of the 07-13 process)

- **Direction:** "look at the security audit ran in the past, can you repeat the same process."
- **Method:** re-applied the `skills-reference/` security skills (Anthropic security-review, owasp-security, insecure-defaults, supply-chain-risk-auditor, coppa-children-privacy) with three parallel reviewers (server / client+dashboard / game+config+privacy), verified every finding against the live code, then fixed server-side and unit-tested — same shape as the 2026-07-13 audit. Full write-up in `SECURITY-AUDIT-2026-07-22.md`.
- **Prior fixes re-verified intact:** PIN brute-force throttle, `crypto.timingSafeEqual` compare, 4-digit-min PIN, baseline response headers, and the July-21 nation-name XSS hardening (`escapeHtml`/`sanitizeNationName`).
- **4 new findings fixed (`coop/coop-server.js` + `coop/dashboard.html`):**
  1. **HIGH — throttle bypass via spoofed `X-Forwarded-For`:** the per-(client+name) throttle keyed on a spoofable header, so rotating the header gave every PIN guess a fresh bucket. Added an IP-independent per-account cap (30 fails / 10 min → 10 min lock) that makes IP spoofing pointless.
  2. **MEDIUM — class-question answer key leaked:** `/__coop/state` returned `correct` + every student's answer to anyone with the room code (a live cheat + classmate-data leak). Now key-gated: students get a new `tqPublicView` (no answer key, no responses); the dashboard sends its teacher key and is unchanged.
  3. **HIGH — stored XSS into the teacher's browser:** `/__coop/sync` only cleaned name/emote, but the dashboard rendered relayed `term`/`grade`/etc. into innerHTML, so a crafted field executed script in the privileged teacher view. Added `sanitizePlayerState()` (coerce numbers, whitelist grades, strip brackets) on ingest, plus `esc()`/`Number()` at the dashboard sinks.
  4. **MEDIUM — teacher key from `Math.random()`:** the sole teacher-auth secret was predictable; switched `randToken()` to `crypto.randomBytes` and class codes to `crypto.randomInt`.
- **Documented, not fixed (recommended next task):** player identity is asserted by a client-supplied `id` (in-session griefing only; durable data stays PIN-gated). Proper fix is a server-issued per-player session token — an architectural change deferred to its own pass, matching how the prior audit deferred deeper items.
- **Privacy:** the four 07-13 recommendations are now largely done in code/content — `coop/privacy.html` published and linked, first-name-only guidance shown, 90-day retention enforced via Redis TTL. Open item remains verifiable parental consent / age gate for under-13 use (policy, not code).
- **Supply chain:** re-confirmed zero runtime/dev dependencies; server is pure Node standard library.
- **Verification:** unit tests `Bug Tests/sec-fix-test-2026-07-22.js` 17/17 passing; live server confirmed the answer-key gate, the 48-char crypto teacher key, and full neutralization of `<img>/<svg>/<script>` sync payloads (zero angle brackets remain); `node --check` clean.
